...
public class CustomPBR : ICustomPBRSubShader
{
    struct Pass
    {
        public string Name;
    ...
    Pass m_ForwardPassMetallic = new Pass
    {
        ...
        {
            CustomPBRNode.AlbedoSlotId,
            CustomPBRNode.NormalSlotId,
            CustomPBRNode.EmissionSlotId,
            CustomPBRNode.MetallicSlotId,
            CustomPBRNode.SmoothnessSlotId,
            CustomPBRNode.OcclusionSlotId,
            CustomPBRNode.AlphaSlotId,
            CustomPBRNode.AlphaThresholdSlotId,
            CustomPBRNode.FresnelTintSlotId,
            CustomPBRNode.FresnelStengthSlotId,
            CustomPBRNode.FresnelPowerSlotId,
            CustomPBRNode.FresnelContributionSlotId,
            CustomPBRNode.TransmissionSlotId
        }
    };
    ...
    private static string GetShaderPassFromTemplate(string template, CustomPBRNode masterNode, Pass pass, GenerationMode mode, SurfaceMaterialOptions materialOptions)
    {
        ...
        if (masterNode.IsSlotConnected(CustomPBRNode.NormalSlotId))
            defines.AddShaderChunk("#define _NORMALMAP 1", true);

        if (masterNode.model == CustomPBRNode.Model.Specular)
            defines.AddShaderChunk("#define _SPECULAR_SETUP 1", true);

        if (masterNode.IsSlotConnected(CustomPBRNode.AlphaThresholdSlotId))
            defines.AddShaderChunk("#define _AlphaClip 1", true);

        if (masterNode.surfaceType == SurfaceType.Transparent && masterNode.alphaMode == AlphaMode.Premultiply)
            defines.AddShaderChunk("#define _ALPHAPREMULTIPLY_ON 1", true);

        if (masterNode.fresnel.isOn)
            defines.AddShaderChunk("#define _Fresnel 1", true);
        else
            defines.AddShaderChunk("#define _Fresnel 0", true);
    ...
    {
        var dirs = Directory.GetDirectories("./Assets/ToonShading");
    ...
    {
        var masterNode = inMasterNode as CustomPBRNode;
